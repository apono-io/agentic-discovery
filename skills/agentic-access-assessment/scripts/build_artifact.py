...
<div class="wrap">
  ...
  <section id="unattr-s" hidden>
    <div class="shead"><h2>Access we could not attribute</h2>
      <p class="lede" id="unattr-lede"></p>
    </div>
    <div class="tablewrap">
      <table>
        <thead><tr><th>Tool</th><th class="num">Actions</th><th class="num">Machines</th></tr></thead>
...
if (D.unresolved && D.unresolved.length) {
  document.getElementById("unattr-s").hidden = false;
  const tot = D.unresolved.reduce((n, u) => n + u.calls, 0);
  const mach = new Set(D.machines.map((m) => m.machine)).size;
  document.getElementById("unattr-lede").textContent =
    tot + " actions reached something external without naming a resource the scanner could " +
    "identify. They are real access, so every count on this page understates the estate rather " +
    "than overstating it. Ordered by volume: the top rows are where a new extraction rule buys " +
    "the most coverage, and they are worth sending back to Apono." +
    (D.unresolvedMachines && D.unresolvedMachines < mach
      ? " Counted from " + D.unresolvedMachines + " of " + mach + " machines \u2014 only reports " +
        "from v0.9 onwards list these, so the real total is higher."
      : "");
  const ub = document.getElementById("ubody");
  for (const u of D.unresolved) {
    const tr = el("tr");
    tr.appendChild(el("td", "mono", u.tool));
    tr.appendChild(el("td", "num", String(u.calls)));
    tr.appendChild(el("td", "num", String(u.machines)));
    ub.appendChild(tr);
  }
}
...
payload = {
    ...
    "unresolvedMachines": data.get("unresolvedMachines", 0),
